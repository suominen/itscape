...
| Debian | 13 (trixie) | 6.12.86-1 | — | :x: Vulnerable — below 6.12.93 |
...
7.0.12) are fixed. **trixie** (`6.12.86-1`) is in-window and below the
first-fixed 6.12.93, so it remains vulnerable until Debian ships ≥ 6.12.93.
...
affected. No AlmaLinux/Rocky/RHEL errata has shipped an ITScape fix as of
this writing, so Rocky 10 aarch64 remains vulnerable; AlmaLinux is the
leading indicator for the fixed el10 kernel. RHEL 10, Oracle Linux 10, and
CloudLinux OS 10 are expected to be in the same state until then.
...
opt-in `kernel6.12` / `kernel6.18` streams **are** in-window; if selected on
a Graviton (arm64) host, verify they carry the backport (6.12.93 / 6.18.35)
...
- **Debian** (via the dak `madison` API): unstable `7.1.3-1` and testing
  `7.0.13-1` carry the fix → fixed; stable `6.12.86-1` is in-window below
  6.12.93 → vulnerable; oldstable `6.1.170-3` and oldoldstable `5.10.223-1`
  predate the v6.10 trigger → not affected.
- **NixOS** (via the local nixpkgs clone): the default `linuxPackages`
  (`linux_6_18`) is `6.18.38` on both channels (≥ 6.18.35) → carries the
  backport → fixed.
- **Rocky / RHEL family**: only el10 (6.12) is in-window; el9 (5.14) and
  el8 (4.18) predate the trigger → not affected. No AlmaLinux/Rocky/RHEL
  errata for CVE-2026-46316 at seed, so Rocky 10 aarch64 → `:x:`.
- **Amazon Linux**: ALAS assessed AL2023 (6.1 default) and AL2 (4.14) **Not
  Affected** (both < 6.10); the opt-in `kernel6.12`/`kernel6.18` streams are
  in-window and unverified.
